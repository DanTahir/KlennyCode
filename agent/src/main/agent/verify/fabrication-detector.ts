...
function checkArtifacts(input: DetectorInput, stripped: string, out: FabricationFinding[]): void {
  ...
  for (const sentence of splitSentences(stripped)) {
    ...
    for (const m of sentence.matchAll(PATH_TOKEN_RE)) {
      ...
      checked++
      const abs = isAbsolute(claimed) ? claimed : resolvePath(root, claimed)
      if (exists(abs)) continue

      out.push({
        code: 'C3',
        severity: 'hard',
        detail: `Message describes creating/writing "${claimed}", but no write tool ever targeted that path in this session and it does not exist on disk (checked ${abs}).`,
        excerpt: excerptOf(sentence)
      })
    }
  }
