...
const MEMORY_TOOL_NOTE_ASSISTANT = `Memory notes: the "Auto-memory index" below lists topic titles like [Some Topic](Some Topic.md) — these are NOT files, so they can't be opened directly. Use read_memory with the exact scope and topic title to load the full note.`
