...
class MockIntersectionObserver implements IntersectionObserver {
  ...
  constructor(callback: IOCallback, options?: IntersectionObserverInit) {
    ...
    // Recorded faithfully (not hardcoded) so a test can assert the ratio an
    // effect asked for — a container far wider than the viewport can never
    // reach a non-zero threshold, so that choice is load-bearing.
...
/**
 * The options every live observer watching `target` was constructed with.
 * Lets a test prove an element is observed at a specific rootMargin/threshold.
 */
export function observerOptionsFor(
  target: Element,
): Array<{ rootMargin: string; thresholds: ReadonlyArray<number> }> {
  return records
    .filter((r) => r.targets.has(target))
    .map((r) => ({
      rootMargin: r.observer.rootMargin,
      thresholds: r.observer.thresholds,
    }));
}
