 ...
 *   4. `extraAssets` from replica.config.json — absolute URLs the capture
 *      viewport never requested (responsive <source>/srcset variants for other
 *      breakpoints). Opt-in, and the only site-specific input here.
 *
...
async function download(url, contentTypeHint, referer) {
  ...
  // Retry genuinely transient throttling/server errors rather than recording
  // them as permanent failures.
  //
  // Scope note: this is defensive hardening, NOT the fix for entity-encoded
  // srcset URLs. A URL harvested with a literal `&amp;` also returns 429 from
  // an image CDN, but deterministically (the resize param is lost, so it reads
  // as a full-resolution request) — retrying that is useless. That bug is
  // fixed at the source, in refsFromHtml's entity decoding in lib/assets.mjs.
  //
  // What this does cover: hundreds of candidates fired at a handful of hosts
  // can trip a real per-host rate limiter a human browsing the page never
  // hits, and a 5xx blip should not permanently drop an asset either.
  //
  // 429/503 often carry `Retry-After`; honour it when it is a sane number of
  // seconds, otherwise back off exponentially with jitter.
  ...
  for (let attempt = 1; ; attempt += 1) {
    ...
    if (!RETRY_STATUS.has(res.status) || attempt >= MAX_ATTEMPTS) {
      return { url, kind, name, error: `HTTP ${res.status}` };
    }

    const after = Number(res.headers.get('retry-after'));
    const waitMs =
      Number.isFinite(after) && after > 0 && after <= 30
        ? after * 1000
...
async function main() {
  ...
  // ---- source 3: config-declared extras ---------------------------------
  // Responsive variants the capture viewport never requested. Kept generic:
  // the script knows nothing about which URLs these are, only that config
  // named them.
  const fromExtras = (cfg.extraAssets ?? []).filter((u) => {
    if (!/^https?:/i.test(u)) {
      console.log(`warning: extraAssets entry is not an absolute URL, skipping: ${u}`);
      return false;
    }
    return !isBlocked(u);
  });

  const queue = [...new Set([...fromNetwork, ...fromMarkup, ...fromExtras])];
  console.log(
    `stage 1: ${queue.length} candidate assets ` +
      `(${new Set(fromNetwork).size} from network log, ${new Set(fromMarkup).size} from markup` +
      `${fromExtras.length ? `, ${new Set(fromExtras).size} from config extraAssets` : ''})`,
  );
