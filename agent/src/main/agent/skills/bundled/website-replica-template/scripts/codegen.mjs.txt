...
const DROP_ATTRS = new Set(['xmlns:svgjs']);
...
async function emitStyles(headInfo) {
  ...
  for (const entry of headInfo.entries ?? []) {
    if (entry.kind === 'link' && /stylesheet/i.test(entry.rel ?? '')) {
      ...
      if (skip) {
        emitted.push({ order: idx, kind: 'link', href: entry.href, status: 'SKIPPED', skipRule: skip });
      ...
      if (!local) {
        emitted.push({ order: idx, kind: 'link', href: entry.href, status: 'MISSING' });
      ...
      try {
        ...
        emitted.push({ order: idx, kind: 'link', href: entry.href, status: 'UNREADABLE' });
        idx += 1;
        continue;
      }
      // Resolve against the sheet's own href, not the page URL.
      const rewritten = rewriteCssUrls(rewriteUrlsInText(css), entry.href);
      ...
      const header =
        ...
        ` * Cascade position ${idx} of the original <head>. Do not edit — \`npm run codegen\`.\n */\n`;
      ...
      const css = rewriteCssUrls(rewriteUrlsInText(entry.css ?? ''), cfg.targetUrl);
  ...
  // Body-level <style> blocks cascade after everything in <head>.
  //
  // Two independent passes observe the same <style> nodes: capture records them
  // (headInfo.bodyStyles, via document.body.querySelectorAll) and the JSX walker
  // collects every one it drops (inlineStylesFromBody). Blindly concatenating
  // both emitted each body stylesheet twice — invisible in the rendered result,
  // since duplicate identical rules cascade to the same outcome, but it doubled
  // the sheet count, inflated the manifest, and made the @import list disagree
  // with the real document. Union them instead, deduped by exact CSS text and
  // preserving document order (capture's order wins).
  const bodyStyleSources = [...(headInfo.bodyStyles ?? []), ...inlineStylesFromBody];
  const seenBodyStyles = new Set();
  const bodyStyles = [];
  for (const raw of bodyStyleSources) {
    const key = (raw ?? '').trim();
    if (!key || seenBodyStyles.has(key)) continue;
    seenBodyStyles.add(key);
    bodyStyles.push(raw);
  }
  for (const [i, raw] of bodyStyles.entries()) {
    const css = rewriteCssUrls(rewriteUrlsInText(raw ?? ''), cfg.targetUrl);
    if (!css.trim()) continue;
