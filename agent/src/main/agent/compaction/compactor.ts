...
function transcriptLineForMessage(m: ChatMessage): string | null {
  ...
  // A message the fabrication guard flagged must never be folded into the summary as though it
  // were trustworthy ground truth — that is exactly the poisoning path this file already guards
  // against for fabricated markers, arriving via a different route: unsupported *prose* claims
  // that were caught at the time but would otherwise be summarized as plain fact and then
  // replayed, unquestioned, on every later turn.
  if (m.verification) {
    const codes = m.verification.findings.map((f) => f.code).join(',')
    const label =
      m.verification.status === 'disputed'
        ? `[DISPUTED by the fabrication guard — claims below contradicted the harness's execution records (${codes}); do NOT record them as things that happened]`
        : `[UNVERIFIED NARRATION — heuristic fabrication warnings (${codes}); treat claims below as unproven]`
    return `${m.role}: ${label} ${line}`
  }

  // Harness-authored audit notes are 'user'-role on the wire but are not user instructions;
  // labelling them keeps the summarizer from recording "the user demanded a retraction".
  if (m.isAuditNote) {
    return `system-audit-note (generated by the harness, not the user): ${line}`
  }
