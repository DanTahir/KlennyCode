...
export function toORMessages(
  ...
  if (compactionSummary) {
    ...
    if (justCompacted) {
      summaryMsg += `\n\nNote: this compaction just happened as part of your current turn, purely to manage context size — it is routine background maintenance, not a stopping point. In your next reply, continue the task exactly where you left off — the next concrete tool call belongs in that same reply. If you want to acknowledge the compaction at all, let one short clause ride along with that tool call; do not send a standalone acknowledgment, since a reply with no tool calls is indistinguishable to the harness from a finished task — the summary's "Next Move" section above is your resumption point — using tool calls as needed. Do not end the turn with just that acknowledgment and no tool calls unless the task was already fully complete before compaction occurred. (If you do stop here while a checklist item is still unfinished, the harness will detect it and resume you automatically, so stopping only costs a wasted round-trip.)`
    }
