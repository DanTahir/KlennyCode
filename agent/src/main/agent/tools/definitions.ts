      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            width: { type: 'number', description: 'Viewport width in CSS pixels, used by resize (240-5120). May be given on its own — the height then stays at a sensible default, which is usually what you want for checking a narrow/mobile layout.' },
            height: { type: 'number', description: 'Viewport height in CSS pixels, used by resize (240-5120).' },
            preset: { type: 'string', enum: ['mobile', 'tablet', 'desktop', 'wide'], description: "Named viewport for resize instead of explicit width/height: 'mobile' (390x844), 'tablet' (820x1180), 'desktop' (1440x900), 'wide' (1920x1080). An explicit width/height overrides the preset's corresponding dimension." },
