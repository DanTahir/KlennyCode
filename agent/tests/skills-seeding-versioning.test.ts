...
// See skills-seeding.test.ts for why node:os is mocked this way (must happen before dataDir.ts,
// which calls homedir(), is loaded anywhere in the process). Each describe block below gets its
// own fake home + fresh dynamic import of manager.ts, since seedBundledSkills() only runs once
// per process (module-level `seedAttempted` latch) — a single shared home across tests would
// only exercise the "first ever seed" path once and skip the rest.
let fakeHome = ''
...
describe('bundled skill versioned re-seeding', () => {
  ...
  beforeAll(async () => {
    ...
    const skillDir = join(fakeHome, '.klenny', 'skills', ...BUNDLED_SKILL_PATH.slice(0, -1))
    ...
    await writeFile(join(fakeHome, '.klenny', '.bundled-skills-seeded'), new Date().toISOString(), 'utf8')
  ...
  test('an untouched legacy install on the most recent marker-only variant is upgraded to the current bundled content', async () => {
    ...
    const state = JSON.parse(await readFile(join(fakeHome, '.klenny', 'skills-seed-state.json'), 'utf8'))
