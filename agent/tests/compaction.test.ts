...
describe('maybeCompact', () => {
  ...
  test('labels a fabrication-guard-disputed message in the transcript so it cannot be summarized as fact', async () => {
    // The poisoning path this closes is the prose analogue of the fabricated-marker one: a
    // message the guard already caught would otherwise be folded into the summary as ordinary
    // narration, and then replayed as trusted ground truth on every later turn.
    const disputed = assistantMsg('a_disputed', 'I scaffolded the whole project and all 22 tests pass.')
    disputed.verification = {
      status: 'disputed',
      findings: [{ code: 'C3', severity: 'hard', detail: 'manage.py does not exist and no write tool targeted it' }]
    }
    const warned = assistantMsg('a_warned', 'Everything is wired up and working end to end.')
    warned.verification = {
      status: 'warned',
      findings: [{ code: 'C5', severity: 'soft', detail: 'many result-shaped claims with no tool calls' }]
    }
    const messages = [
      ...buildMessages(10),
      disputed,
      warned,
      ...buildMessages(20),
      assistantMsg('a_last2', 'ok', { promptTokens: 900_000, completionTokens: 10 })
    ]
    const result = await maybeCompact({ messages, model, apiKey: 'k', utilityModel: 'test/model', models: [model] })
    expect(result.compacted).toBe(true)
    const summary = result.summary ?? ''
    expect(summary).toContain('[DISPUTED by the fabrication guard')
    expect(summary).toContain('C3')
    expect(summary).toContain('[UNVERIFIED NARRATION')
    expect(summary).toContain('C5')
    // The claims themselves must still be present (labelled, not censored) so the summarizer can
    // record what was claimed while knowing it was contradicted.
    expect(summary).toContain('I scaffolded the whole project')
  })

  test('labels a harness-authored audit note as a system note, not as user instruction', async () => {
    // Audit notes are role:'user' purely because toORMessages() only emits system messages for
    // the prompt/summary prefix. Unlabelled, a summary could record "the user demanded a
    // retraction" — inventing a user statement that never happened.
    const auditNote: ChatMessage = {
      id: 'audit_1',
      role: 'user',
      blocks: [{ type: 'text', text: 'Automated audit: retract the unsupported claims above.' }],
      createdAt: Date.now(),
      isAuditNote: true
    }
    const messages = [
      ...
      auditNote,
      ...buildMessages(20),
      assistantMsg('a_last3', 'ok', { promptTokens: 900_000, completionTokens: 10 })
    ]
    const result = await maybeCompact({ messages, model, apiKey: 'k', utilityModel: 'test/model', models: [model] })
    expect(result.compacted).toBe(true)
    const summary = result.summary ?? ''
    expect(summary).toContain('system-audit-note (generated by the harness, not the user)')
    expect(summary).not.toContain('user: Automated audit:')
  })
