...
- **Browser automation (beta)** — optional, off-by-default local Playwright-driven browser control (navigate, click, type, snapshot, screenshot, viewport resize, etc. — including `mobile`/`tablet`/`desktop`/`wide` presets for checking responsive layouts), gated by its own policy (Off/Ask/Auto) in Settings → Automation, independent of file-edit approvals. Chromium isn't bundled with the app; the very first browser session downloads it once (~150 MB), showing progress inline in the tool call
...
- **Self-diagnostics log** — the app also captures its *own* main-process output (prompt-cache decisions, provider errors, stack traces) to a size-capped, ANSI-stripped `process.log`, readable by the agent via a `read_app_log` tool. This works even in the packaged app, which has no console attached, so the agent can diagnose and verify fixes to Klenny Code itself instead of asking you to run a build from a terminal and paste the output back
- **Selectable shell** — pick which shell `run_command` and the terminal use (e.g. Git Bash, PowerShell, cmd, WSL) in Settings, or leave it on OS-default Auto. On macOS/Linux the app picks up your login shell's `PATH` at startup, so Homebrew/nvm/bun tools are found even when the app is launched from Finder or the Dock
- **Native menus** — a standard App/Edit/View/Window menu bar on macOS (so ⌘C/⌘V/⌘X/⌘A work everywhere), plus right-click Cut/Copy/Paste menus (with spelling suggestions) in text fields and a Copy/Paste/Select All/Clear menu in the terminal on every platform
